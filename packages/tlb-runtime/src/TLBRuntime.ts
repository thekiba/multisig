...
export class TLBRuntime<T extends ParsedCell = ParsedCell> {
  private readonly tagMap = new Map<
    string,
    { type: TLBType; item: TLBConstructor }
  >();
  ...
  static from<T extends ParsedCell = ParsedCell>(
    ...
    try {
      ...
      const lastTypeName = pared[pared.length - 1]
        .split(";")[0]
        .trim()
        .split(" ")[0]
        .trim();
  ...
  private findByTag(
    slice: Slice,
  ): { type: TLBType; item: TLBConstructor } | null {
  ...
  deserialize(data: Cell | string, findByTag = false): Result<T> {
    ...
    try {
      const result = this.deserializeByTypeName(
        this.lastTypeName,
        slice.clone(),
      );
      if (result.ok) {
  ...
  private deserializeConstructor(
    ...
    const kind =
      type.constructors.length > 1
        ? `${type.name}_${constructor.name}`
        : type.name;
    ...
    if (args.length > 0 && constructor.parameters.length > 0) {
      ...
      for (
        let i = 0;
        i < Math.min(args.length, constructor.parameters.length);
        i++
      ) {
        ...
        try {
          if (param.variable?.name && param.variable.deriveExpr) {
            const derived = new MathExprEvaluator(variables).evaluate(
      ...
      if (field.subFields.length > 0) {
        if (slice.remainingRefs === 0) {
          return {
            ok: false,
            error: new TLBDataError(
              `No more references available for field ${field.name}`,
            ),
          };
        }
          ...
          if (subfield.fieldType.kind === "TLBCellType") {
            ...
            if (type) {
              ...
              return {
                ok: false,
                error: new TLBDataError(
                  `Type ${subfield.fieldType.name} not found`,
                ),
              };
            }
          } else {
            ...
            const refSlice = ref.beginParse(true);
            value[field.name] = this.deserializeField(
              subfield,
              refSlice,
              variables,
            );
    ...
    for (const constraint of constructor.constraints) {
      if (evaluator.evaluate(constraint) !== 1) {
        return {
          ok: false,
          error: new TLBDataError(
            `Failed to deserialize type ${kind} due to constraint`,
          ),
        };
      }
    }
    ...
    if (
      ...
      (value as Record<string, unknown>).x = BigInt(
        (value as Record<string, number>).x,
      );
    }
  ...
  private deserializeFieldType(
    ...
    switch (fieldType.kind) {
      case "TLBNumberType": {
        ...
        try {
          ...
          if (e instanceof Error) {
            const m = /Variable\s+([^\s]+)\s+is\s+not\s+defined/.exec(
              e.message,
            );
            if (m && m[1]) {
              const name = m[1];
      ...
      case "TLBNamedType": {
        ...
        if (!type) {
          throw new TLBDataError(
            `Type ${fieldType.name} not found in TL-B schema`,
          );
        }
      ...
      case "TLBCellType": {
        if (slice.remainingRefs === 0) {
          throw new TLBDataError(
            "No more references available for TLBCellType",
          );
        }
      ...
      case "TLBCellInsideType": {
        if (slice.remainingRefs === 0) {
          throw new TLBDataError(
            "No more references available for TLBCellInsideType",
          );
        }
        const ref = slice.loadRef();
...
export function parseTLB<T extends ParsedCell = ParsedCell>(
  schema: string,
): TLBRuntime<T> {
